...
import {
  addressToCoordinatesGoogle,
  calculateDemandInfrastructureScore,
  calculateSuburbCBDScore,
} from "../utils/map";
...
const getRentalPrice = (property: Doc<"properties">): number | null => {
...
function getYear(record: Doc<"absMarketData">) {
  return parseInt(record.census_year.toString());
}
...
function normalizeToScale(
  ...
  const normalized =
    ((clamped - min) / (max - min)) * (scaleMax - scaleMin) + scaleMin;
...
function calculatePriceGrowth(sales: Doc<"properties">[]) {
  ...
  const cagr = Math.pow(endMedian / startMedian, 1 / yearsDiff) - 1;
...
function calculateIncomeGrowth(absData: Doc<"absMarketData">[]) {
  ...
  const years =
    new Date(sorted[sorted.length - 1].scrapedAt!).getFullYear() -
    new Date(sorted[0].scrapedAt!).getFullYear();
...
function calculateAffordabilityScore(
  ...
  const score = ((maxRatio - clamped) / (maxRatio - minRatio)) * 100;
...
function calculateProfessionalWorkforcePercentage(
  ...
  for (const year of Object.keys(grouped)) {
    ...
    if (totalEmployed > 0) {
      yearlyPercentages.push((totalProfessionals / totalEmployed) * 100);
    }
  }
  ...
  const avg =
    yearlyPercentages.reduce((a, b) => a + b, 0) / yearlyPercentages.length;
...
function calculateRentalGrowthScore(rentalProperties: Doc<"properties">[]) {
  ...
  for (let i = 1; i < years.length; i++) {
    ...
    const currAvgPrice =
      currPrices.reduce((sum, p) => sum + p, 0) / currPrices.length;

    if (prevAvgPrice > 0) {
      yearlyGrowthRates.push(
        ((currAvgPrice - prevAvgPrice) / prevAvgPrice) * 100,
      );
    }
  }
  ...
  // Average across years
  const avgGrowthRate =
    yearlyGrowthRates.reduce((a, b) => a + b, 0) / yearlyGrowthRates.length;
...
function calculatePriceVolatility(sales: Doc<"properties">[]) {
...
function calculateInterestRateSensitivity(
  ...
  if (!weeklyIncome || !monthlyMortgage || !typicalValue) return null;

  const monthlyIncome = (weeklyIncome * 52) / 12;

  const mortgageBurden = monthlyMortgage / monthlyIncome; // ratio

  const priceToIncome = typicalValue / (weeklyIncome * 52);

  // Mortgage ownership ratio
  const mortgagedOwners =
    latest.tenureType?.find((t) => t.label.toLowerCase().includes("mortgage"))
      ?.count ?? 0;

  const totalTenure = (latest.ownerOccupied ?? 0) + (latest.rented ?? 0);

  const mortgageRatio = totalTenure > 0 ? mortgagedOwners / totalTenure : 0;

  // Normalize each risk component (example ranges)
  const burdenScore = normalizeToScale(mortgageBurden, 0.15, 0.45, 0, 100)!;

  const priceIncomeScore = normalizeToScale(priceToIncome, 4, 12, 0, 100)!;

  const leverageScore = normalizeToScale(mortgageRatio, 0.2, 0.6, 0, 100)!;

  // Composite (higher = more sensitive)
  const sensitivity =
    burdenScore * 0.4 + priceIncomeScore * 0.35 + leverageScore * 0.25;
...
function calculateLiquidityRisk({
  ...
  const domScore = normalizeToScale(averageDaysOnMarket, 20, 120, 0, 100)!;

  const stockScore = normalizeToScale(stockOnMarketPct, 1, 8, 0, 100)!;

  const auctionScore = normalizeInverse(auctionClearanceRate, 50, 85)!;
...
function calculateConcentrationRisk(absData: Doc<"absMarketData">) {
  const industryHHI = calculateHHI(absData.industryTopResponses ?? []);

  const dwellingHHI = calculateHHI(absData.dwellingStructure ?? []);

  const renterShare =
    absData.rented && absData.ownerOccupied
      ? absData.rented / (absData.rented + absData.ownerOccupied)
      : null;

  const renterScore =
    renterShare !== null ? normalizeToScale(renterShare, 0.2, 0.7, 0, 100) : 0;
...
const normalizeInverse = (
  ...
  const clamped = Math.min(Math.max(value, min), max);
  const normalized =
    ((max - clamped) / (max - min)) * (scaleMax - scaleMin) + scaleMin;
  return normalized;
};
...
const ratioScore = (actual: number, ideal: number) => clamp(actual / ideal);
...
async function fetchAllSoldPropertiesForSuburb(
  ...
  try {
    ...
    console.error(`Failed to fetch properties for suburb ${suburb}:`, error);
    throw error;
  }
}
...
async function fetchAllAbsSuburbData(suburb: string, postcode: string) {
  "use step";
  try {
    const absDemographicData = await client.query(
      api.functions.absMarketData.getAbsMarketDataByPostcode,
      { postcode },
    );
    ...
    console.error(
      `Failed to fetch ABS suburb data for suburb ${suburb}:`,
      error,
    );
    throw error;
  }
...
async function calculateSuburbMetrics(
  ...
  const totalTenure =
    avgRenters !== null && avgOwners !== null ? avgRenters + avgOwners : null;
  ...
  const annualExpenses = typicalValue !== null ? typicalValue * 0.1 : null;
  ...
  const salesVolumeScore = ratioScore(salesCount, THRESHOLDS.idealSales);
  ...
  const salesScore = salesVolumeScore * 0.6 + salesFieldCompleteness * 0.4;

  const rentalVolumeScore = ratioScore(rentalCount, THRESHOLDS.idealRentals);
  ...
  const rentalScore = rentalVolumeScore * 0.6 + rentalFieldCompleteness * 0.4;

  const samplePenalty = salesCount < 10 || rentalCount < 8 ? 0.6 : 1;

  const dataCompletenessScore = Math.round(
    (salesScore * 0.55 + rentalScore * 0.35 + samplePenalty * 0.1) * 100,
  );
  ...
  const priceGrowthScore =
    priceGrowth !== null ? normalizeToScale(priceGrowth, -5, 15) : null;

  const incomeGrowth = calculateIncomeGrowth(absData);
  const incomeGrowthScore =
    incomeGrowth !== null ? normalizeToScale(incomeGrowth, 0, 10) : null;
  ...
  const state = suburbParts.length > 1 ? suburbParts[1].trim() : null;

  const cbdProximityResult = state
  ...
  const avgWeeklyIncome = safeAverage(
    absData.map((d) => d.medianWeeklyHouseholdIncome ?? 0).filter((v) => v > 0),
  );

  const avgAnnualIncome = avgWeeklyIncome ? avgWeeklyIncome * 52 : null;

  const affordabilityResult =
    typicalValue && avgAnnualIncome
      ? calculateAffordabilityScore(typicalValue, avgWeeklyIncome!)
      : null;
  ...
  const capitalGrowthComponent = [
    ...
    { score: incomeGrowthScore, weight: 0.2 },
    { score: infrastructurePipelineScore, weight: 0.15 },
    { score: employmentScore, weight: 0.15 },
    { score: cbdProximityScore, weight: 0.1 },
    { score: auctionScore, weight: 0.1 },
  ...
  const capitalGrowthScoreRaw =
    totalWeight > 0
      ? validComponents.reduce((sum, c) => sum + c.score! * c.weight, 0) /
        totalWeight
      : null;

  const capitalGrowthScore =
    capitalGrowthScoreRaw !== null
      ? Math.round(clamp(capitalGrowthScoreRaw, 0, 100))
      : null;

  // ---------- Cash Flow Score Calculation ----------
  const grossYeildScore =
    netYield !== null ? normalizeToScale(netYield, 0, 10) : null;
  const vacancyScore =
    vacancyRate !== null ? normalizeToScale(vacancyRate, 0, 10) : null;
  const rentalGrowthScore = calculateRentalGrowthScore(renterProperties);
  const daysOnMarketScore =
    averageDaysOnMarket !== null
      ? normalizeToScale(averageDaysOnMarket, 90, 0)
      : null;
  const stockOnMarketScore =
    stockOnMarket !== null ? normalizeToScale(stockOnMarket, 0, 0.05) : null;

  const cashFlowComponent = [
    { score: grossYeildScore, weight: 0.3 },
    { score: vacancyScore, weight: 0.25 },
    { score: rentalGrowthScore, weight: 0.15 },
    { score: daysOnMarketScore, weight: 0.1 },
    { score: stockOnMarketScore, weight: 0.1 },
    { score: affordabilityScore, weight: 0.1 },
  ];
  ...
  const cashFlowScore =
    cashFlowScoreRaw !== null
      ? Math.round(clamp(cashFlowScoreRaw, 0, 100))
      : null;
  ...
  else if (averageDaysOnMarket && averageDaysOnMarket > 60)
    risk.marketRisk += 20;
  else if (averageDaysOnMarket && averageDaysOnMarket > 30)
    risk.marketRisk += 10;
  else risk.marketRisk += 5;
  ...
  const volatilityScore =
    priceVolatility !== null
      ? normalizeToScale(priceVolatility, 0, 0.15)
      : null;
  risk.marketRisk +=
    volatilityScore !== null ? (volatilityScore / 100) * 30 : 0;

  const avgMonthlyMortgage =
    absData.reduce((sum, d) => sum + (d.medianWeeklyHouseholdIncome ?? 0), 0) /
    absData.filter((d) => d.medianMonthlyMortgageRepayment != null).length;
  const loanToValueRatio =
    typicalValue && avgMonthlyMortgage
      ? (avgMonthlyMortgage * 12) / typicalValue
      : null;
  ...
  const interestRateSensitivity = calculateInterestRateSensitivity(
    absData,
    typicalValue!,
  );
  risk.financialRisk +=
    interestRateSensitivity !== null ? (interestRateSensitivity * 30) / 100 : 0;
  ...
  const latestAbsData = absData.reduce<Doc<"absMarketData"> | null>(
    (latest, current) => {
      ...
      return current.scrapedAt > latest.scrapedAt ? current : latest;
    },
    null,
  )!;
  ...
  const riskComponents = [
    ...
    { score: risk.concentrationRisk, weight: 0.1 },
  ];
  const totalRiskWeight = riskComponents.reduce((sum, c) => sum + c.weight, 0);
  const riskScoreRaw =
    totalRiskWeight > 0
      ? riskComponents.reduce((sum, c) => sum + c.score * c.weight, 0) /
        totalRiskWeight
      : null;
  const riskScore =
    riskScoreRaw !== null ? Math.round(clamp(riskScoreRaw, 0, 100)) : null;
...
async function updateSuburbMetricsInDb(
  metrics: Awaited<ReturnType<typeof calculateSuburbMetrics>>,
  suburb: string,
  postcode: string,
  state: string,
) {
  "use step";
  try {
    // const suburbId = await client.query(
    //   api.functions.suburb.getSuburbIdByName,
    //   { postcode },
    // );
    const geometry = await addressToCoordinatesGoogle(
...
export async function suburbAsyncWorkflow(
  ...
  const processLocation = async (loc: {
    ...
    try {
      const [soldProperties, renterProperties, saleProperties, absData] =
        await Promise.all([
          fetchAllSoldPropertiesForSuburb(loc.suburb, loc.state, loc.postcode),
          fetchAllRentPropertiesForSuburb(loc.suburb, loc.state, loc.postcode),
          fetchAllSalePropertiesForSuburb(loc.suburb, loc.state, loc.postcode),
      ...
      console.error(`Workflow failed for suburb ${loc.suburb}:`, message);
  ...
  try {
    ...
    for (let i = 0; i < locations.length; i += CONCURRENCY_LIMIT) {
      ...
      await Promise.all(batch.map((loc) => processLocation(loc)));
    }
