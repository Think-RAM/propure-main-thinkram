...
│  │ • Discovery Qs  │       │ • Financial     │       │ • Data fetching │        │
│  │ • Strategy ID   │       │   calculations  │       │ • Market data   │        │
│  │ • Goal setting  │       │ • Risk analysis │       │ • Suburb stats  │        │
│  │ • Constraints   │       │ • ROI/cashflow  │       │ • Property info │        │
│  │                 │       │ • Scoring       │       │                 │        │
│  │ Tools:          │       │ Tools:          │       │ Tools:          │        │
│  │ • askQuestion   │       │ • calculateROI  │       │ • getSuburbStats│        │
│  │ • setStrategy   │       │ • assessRisk    │       │ • searchProps   │        │
│  │ • updateGoals   │       │ • projectCashFlow│      │ • getMetrics    │        │
│  │ • setFilters    │       │ • scoreProperty │       │ • fetchExternal │        │
│  └─────────────────┘       └─────────────────┘       └─────────────────┘        │
│         │                          │                          │                  │
│         └──────────────────────────┼──────────────────────────┘                  │
│                                    │                                             │
...
│  ├─────────────────────────────────────────────────────────────────────────┤    │
│  │                                                                          │    │
│  │  DATABASE TOOLS              AI TOOLS               UI TOOLS            │    │
│  │  ──────────────              ────────               ────────            │    │
│  │  • searchProperties          • embedText            • updateFilters     │    │
│  │  • getSuburbStats            • similaritySearch     • highlightSuburbs  │    │
│  │  • getPropertyById           • generateSummary      • zoomToArea        │    │
│  │  • getMetricsHistory                                • showProperty      │    │
│  │  • saveSearch                CALCULATION TOOLS      • updateList        │    │
│  │                              ─────────────────                           │    │
│  │  EXTERNAL TOOLS              • calculateROI                             │    │
│  │  ──────────────              • projectCashFlow                          │    │
│  │  • fetchDomainData           • assessAffordability                      │    │
│  │  • fetchABSStats             • scoreStrategy                            │    │
│  │  • fetchRBAData                                                         │    │
│  │                                                                          │    │
│  └─────────────────────────────────────────────────────────────────────────┘    │
│                                                                                  │
...
```typescript
// packages/ai/src/agents/orchestrator.ts
...
const ORCHESTRATOR_SYSTEM_PROMPT = `You are the Propure AI orchestrator. Your role is to help users find the right property investment strategy and properties that match their goals.

You coordinate between three specialist agents:
1. STRATEGIST - For strategy discovery, goal setting, and user profiling
2. ANALYST - For financial calculations, risk assessment, and property scoring
3. RESEARCHER - For fetching market data, suburb statistics, and property information

Based on the user's message, decide which agent(s) to invoke and how to combine their outputs.

CONVERSATION PHASES:
1. DISCOVERY - Use Strategist to understand user's situation, goals, constraints
2. STRATEGY - Use Strategist to recommend a strategy, Analyst to validate feasibility
3. SEARCH - Use Researcher to find matching suburbs/properties, Analyst to score them
4. REFINEMENT - Iterate based on user feedback, adjust filters
5. SELECTION - Deep dive on specific properties, provide comprehensive analysis

IMPORTANT:
- Always explain your reasoning to the user
- When you update filters or search, call the updateUI tool to refresh the map/list
  ...
  const result = await streamText({
    ...
    tools: {
      ...
      invokeStrategist: tool({
        description: 'Ask the Strategist agent for strategy guidance, discovery questions, or goal setting',
        parameters: z.object({
          task: z.string().describe('What you want the Strategist to do'),
          currentStrategy: z.any().optional(),
        }),
        execute: async ({ task, currentStrategy }) => {
          return strategistAgent.run(task, { ...context, strategy: currentStrategy })
        },
      }),

      invokeAnalyst: tool({
        description: 'Ask the Analyst agent for financial calculations, risk assessment, or property scoring',
        parameters: z.object({
          task: z.string().describe('What you want the Analyst to do'),
          data: z.any().describe('Relevant data for the analysis'),
        }),
        execute: async ({ task, data }) => {
          return analystAgent.run(task, { ...context, data })
        },
      }),

      invokeResearcher: tool({
        description: 'Ask the Researcher agent to fetch market data, suburb stats, or property information',
        parameters: z.object({
          task: z.string().describe('What data to fetch'),
          params: z.any().optional(),
        }),
        execute: async ({ task, params }) => {
          return researcherAgent.run(task, { ...context, params })
        },
      }),
      ...
      // UI update tool
      updateUI: tool({
        description: 'Send an update to the frontend UI (map, list, filters)',
        parameters: z.object({
          type: z.enum(['filters', 'highlight', 'zoom', 'showProperty', 'updateList']),
          payload: z.any(),
        }),
        execute: async ({ type, payload }) => {
          onUIUpdate({ type, payload })
          return { success: true }
        },
      }),
    },
...
```typescript
// packages/ai/src/agents/strategist.ts

import { anthropic } from '@ai-sdk/anthropic'
import { generateText, tool } from 'ai'
import { z } from 'zod'

const STRATEGIST_SYSTEM_PROMPT = `You are a property investment strategist. Your role is to:

1. DISCOVER the user's situation through thoughtful questions:
   - Financial: income, savings, existing equity, borrowing capacity
   - Goals: primary objective (income, growth, quick profit), timeline
   - Personal: risk tolerance, time for management, skills (reno?)
   - Constraints: location, budget, property type preferences

2. RECOMMEND the right investment strategy:
   - CASH_FLOW: Focus on rental yield, positive gearing
   - CAPITAL_GROWTH: Focus on appreciation, negative gearing
   - RENOVATION: Buy, improve, sell/refinance
   - DEVELOPMENT: Subdivide or build
   - SMSF: Self-managed super fund compliant
   - COMMERCIAL: Office/retail/industrial
   - MIXED: Combination strategies

3. SET appropriate search filters based on strategy

Ask ONE focused question at a time. Be conversational but efficient.
When you have enough information, recommend a strategy with clear rationale.`
    ...
    const result = await generateText({
      ...
      tools: {
        askDiscoveryQuestion: tool({
          description: 'Ask the user a discovery question',
          parameters: z.object({
            question: z.string(),
            category: z.enum(['financial', 'goals', 'personal', 'constraints']),
            options: z.array(z.string()).optional(),
          }),
        }),

        recommendStrategy: tool({
          description: 'Recommend an investment strategy to the user',
          parameters: z.object({
            strategy: z.enum([
              'CASH_FLOW', 'CAPITAL_GROWTH', 'RENOVATION',
              'DEVELOPMENT', 'SMSF', 'COMMERCIAL', 'MIXED'
            ]),
            rationale: z.string(),
        ...
        updateUserProfile: tool({
          description: 'Update the user profile with discovered information',
          parameters: z.object({
            field: z.string(),
            value: z.any(),
          }),
        ...
        setSearchFilters: tool({
          description: 'Set search filters based on strategy',
          parameters: z.object({
...
import { GeoQueries } from '@propure/geo'

export const tools = {
  database: {
    searchProperties: tool({
      description: 'Search for properties matching filters',
      parameters: z.object({
        filters: z.object({
          suburbIds: z.array(z.string()).optional(),
          priceMin: z.number().optional(),
      ...
      execute: async ({ filters, limit }) => {
        const geoQueries = new GeoQueries(db)
        const properties = await geoQueries.searchProperties(filters, limit)
        return { count: properties.length, properties }
      },
    }),

    getSuburbStats: tool({
      description: 'Get statistics for a suburb',
      parameters: z.object({
      ...
      execute: async (params) => {
        const suburb = await db.suburb.findFirst({
          where: params.suburbId
            ? { id: params.suburbId }
            : params.postcode
            ? { postcode: params.postcode }
            : { name: { contains: params.suburbName, mode: 'insensitive' } },
          include: { city: { include: { state: true } } },
        })

        if (!suburb) return { error: 'Suburb not found' }
        ...
        return { suburb, metrics }
      },
    }),

    getMetricsHistory: tool({
      description: 'Get historical metrics for a suburb',
      parameters: z.object({
        suburbId: z.string(),
        metricTypes: z.array(z.string()),
        period: z.enum(['1y', '2y', '5y', '10y']).default('2y'),
      }),
      execute: async ({ suburbId, metricTypes, period }) => {
        const intervals = { '1y': '1 year', '2y': '2 years', '5y': '5 years', '10y': '10 years' }
        ...
        return db.$queryRaw`
          ...
          AND time > NOW() - INTERVAL ${intervals[period]}
  ...
  calculations: {
    calculateROI: tool({
      description: 'Calculate ROI and cash flow for a property',
      parameters: z.object({
        ...
        expenses: z.object({
          ...
          strata: z.number().optional(),
          insurance: z.number().optional(),
          maintenance: z.number().optional(),
          managementFee: z.number().optional(), // percentage
        }).optional(),
      }),
      execute: async ({ purchasePrice, deposit, interestRate, weeklyRent, expenses }) => {
        const loanAmount = purchasePrice - deposit
        const monthlyInterest = (loanAmount * (interestRate / 100)) / 12
        const annualRent = weeklyRent * 52

        const annualExpenses =
          (expenses?.councilRates || purchasePrice * 0.004) +
          (expenses?.strata || 0) +
          (expenses?.insurance || purchasePrice * 0.001) +
          (expenses?.maintenance || annualRent * 0.05) +
          (expenses?.managementFee ? annualRent * (expenses.managementFee / 100) : annualRent * 0.08)

        const annualInterest = monthlyInterest * 12
        const netIncome = annualRent - annualExpenses - annualInterest
        ...
        return {
          ...
          monthlyCashFlow: cashFlow.toFixed(0),
          annualCashFlow: netIncome.toFixed(0),
          breakdown: {
            annualRent,
            annualExpenses,
            annualInterest,
    ...
    scorePropertyForStrategy: tool({
      description: 'Score a property against a strategy',
      parameters: z.object({
        propertyId: z.string(),
        strategyType: z.enum(['CASH_FLOW', 'CAPITAL_GROWTH', 'RENOVATION', 'MIXED']),
      }),
      execute: async ({ propertyId, strategyType }) => {
        ...
        if (strategyType === 'CASH_FLOW') {
          // Yield is most important
          const yieldScore = Math.min(100, (property.suburb.grossYield || 0) * 20)
          factors.push({ name: 'Gross Yield', score: yieldScore, weight: 0.4 })

          // Low vacancy
          const vacancyScore = Math.max(0, 100 - (property.suburb.vacancyRate || 5) * 20)
          factors.push({ name: 'Low Vacancy', score: vacancyScore, weight: 0.3 })

          // Rental growth
          factors.push({ name: 'Rental Growth', score: 70, weight: 0.2 }) // Would calculate from metrics

          // Price relative to rent
          factors.push({ name: 'Price Efficiency', score: 65, weight: 0.1 })
        }

        if (strategyType === 'CAPITAL_GROWTH') {
          // Historical growth
          factors.push({ name: 'Historical Growth', score: property.suburb.capitalGrowthScore || 50, weight: 0.35 })

          // Infrastructure pipeline
          factors.push({ name: 'Infrastructure', score: 60, weight: 0.25 })

          // Demographics
          factors.push({ name: 'Demographics', score: 70, weight: 0.2 })

          // Scarcity
          factors.push({ name: 'Land Scarcity', score: 75, weight: 0.2 })
        }
