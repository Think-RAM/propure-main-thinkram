...
```typescript
export const handleLegendExtraction = async (
  url: string
): Promise<Styles[]> => {
  ...
  const layerLegend = legendJson.layers.find(
...
export const styleLayer = (feature: any, legends: Styles[]) => {
  ...
  const legendItem = legends.find(
    (l) => l.idKey.includes(key) || l.label === key
  );
...
useEffect(() => {
  ...
  const updateSpatialFilter = () => {
    ...
    layerRef.current!.query().bboxIntersects(bounds).where("1=1");
