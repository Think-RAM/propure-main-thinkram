...
export const strategistOutputSchema = z.object({
    strategyDiscovery: z.object({
        summary: z.string().describe("High-level strategy direction and rationale."),
        hypotheses: z
            .array(z.string())
            .min(1)
...
const StrategyAgent = ({ user, strategyId }: Omit<ResearcherAgentProps, "dataStream">) => {
...
export const StrategyAgentTool = ({ user, strategyId, dataStream }: ResearcherAgentProps) => {
    return tool({
